...
/**
 * PulseRender - ScatterPlot Component
 *
 * Renders multi-category point clouds using Canvas, driven by the shared rAF scheduler.
 * Consumes derived filtered/aggregated data across categories and supports zoom & pan.
 */

import { useEffect, useRef, useMemo } from 'react';

import type { ScatterChartConfig } from '@/lib/types';
...
import { renderScatterPlot, DEFAULT_SCATTER_CONFIG } from '@/lib/rendering/charts/scatter';
...
interface ScatterPlotProps {
  readonly config: ScatterChartConfig;
}

export function ScatterPlot({ config }: ScatterPlotProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const renderConfig = {
      ...
      pointRadius: config.pointRadius ?? DEFAULT_SCATTER_CONFIG.pointRadius,
      yDomain: config.yDomain ?? DEFAULT_SCATTER_CONFIG.yDomain,
    ...
    const unregister = registerRenderer(config.id, () => {
      ...
      renderScatterPlot(ctx, points, dims, renderConfig);
